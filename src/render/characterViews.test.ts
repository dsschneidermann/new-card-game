...
describe('buildCharacterViews', () => {
  it('composes the right-facing anim, mirroring + scaling from world state', () => {
    const world = createWorld(1);
    const e = world.createEntity();
    world.store(HexPosition).add(e, { hex: { q: 0, r: 0 } });
    world.store(FacingState).add(e, { facing: 'left' });
    world.store(Renderable).add(e, { texture: 'player.idle', animBase: 'player', scale: 0.5 });

    // idle when no MovePath; facing left -> mirrored
    let views = [...buildCharacterViews(world, LAYOUT)];
    expect(views).toHaveLength(1);
    expect(views[0]).toMatchObject({ anim: 'player.idle.right', flipX: true, scale: 0.5 });
